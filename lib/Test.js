...
function Test(param) { // @arg Object: { disable, browser, worker, node, button, both, primary, secondary }
                       ...
                       // @help: Test
//{@assert
    _if(!param || param.constructor !== ({}).constructor, "param is not Object", Test, "param");
    _if(param && !param["primary"], "param.primary is null", Test, "param");
//}@assert
...
function Test_run(callback) { // @arg Function(= null): callback(err:Error)
    ...
    if (that._param["both"] && that._param["secondary"]) {
...
//{@assert
function _if(value, msg, fn, keyword) {
    if (value) {
        console.error(Valid.stack(msg));
        if (fn && keyword && global["Help"]) {
            global["Help"](fn, keyword);
        }
        throw new Error(msg);
    }
}

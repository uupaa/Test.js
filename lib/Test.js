...
function Test_run(callback) { // @arg Function(= null): callback(err:Error)
    ...
    var route = "primary_node > primary_browser > primary_worker";

    if (that._param["secondary"]) {
        route += "> 1000 > swap > secondary_node > secondary_browser > secondary_worker > undo";
    }

    Task.run(route, {
        primary_node:       function(task) { _node(that, task); },
        primary_browser:    function(task) { _browser(that, task); },
        primary_worker:     function(task) { _worker(that, task); },
        swap:               function(task) { _swap(that); task.pass(); },
        secondary_node:     function(task) { _node(that, task); },
        secondary_browser:  function(task) { _browser(that, task); },
        secondary_worker:   function(task) { _worker(that, task); },
        undo:               function(task) { _undo(that); task.pass(); }
    }, function(err) {
        if (callback) {
            callback(err);
        }

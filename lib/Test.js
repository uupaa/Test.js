...
function _testRunner(that, finishedCallback) { // @are Function(= null):
    ...
    function _next() {
        ...
        if (fn) {
            ...
            var nextObject = {};
            var testItemName = fn["name"] || (fn + "").split(" ")[1].split("\x28")[0];
            ...
            nextObject["pass"] = function() {
                _passLogger(order, testItemName);
                nextObject = null;
                task.pass();
            };
            nextObject["miss"] = function() {
                _missLogger(order, testItemName);
                nextObject = null;
                task.miss();
            };
            fn(nextObject);
...
function _passLogger(order, testItemName) {
    if (global["console"] && global["console"]["log"]) {
        if (_inNode) {
            console.log(_CONSOLE_COLOR.GREEN + "Node(" + order + "): " + _CONSOLE_COLOR.CLEAR + testItemName);
        } else if (_inWorker) {
            console.log("Worker(" + order + "): " + testItemName);
        } else if (_inBrowser) {
            if (_stylish) {
                console.log("%cBrowser(" + order + "):%c", "color:#0c0", "", testItemName);
            } else {
                console.log("Browser(" + order + "):", testItemName);
            }
        }
    }
}

function _missLogger(order, testItemName) {
    if (global["console"] && global["console"]["error"]) {
        if (_inNode) {
            console.log(_CONSOLE_COLOR.RED + "Node(" + order + "): " + _CONSOLE_COLOR.CLEAR + testItemName);
        } else if (_inWorker) {
            console.error("Worker(" + order + "): " + testItemName);
        } else if (_inBrowser) {
            if (_stylish) {
                console.error("%cBrowser(" + order + "):%c", "color:red", "", testItemName);
            } else {
                console.error("Browser(" + order + "):", testItemName);
            }
        }
    }
}

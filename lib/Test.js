...
Test["swap"]                = Test_swap;   // Test.swap(base:Object/Function, target:Object/Function):UndoObject
Test["undo"]                = Test_undo;   // Test.undo(undoObject:Object):null
...
function Test_swap(base,     // @arg Object/Function: base object.
                   target) { // @arg Object/Function: target object.
                             // @ret UndoObject: { base, name }
//{@assert
    _if(base.constructor !== ({}).constructor && typeof base !== "function", "invalid Test#swap(base)");
//}@assert

    var undo = { base: base, name: base.name || "" };

    if (target) {
        global[undo.name] = target; // global["base"] = target;
    }
    return undo;
}

function Test_undo(undo) { // @arg UndoObject: { base, name }
                           // @ret null:
//{@assert
    _if(!undo || !("base" in undo) || !("name" in undo) ||
        (undo.base.constructor !== ({}).constructor && typeof undo.base !== "function") ||
        typeof undo.name !== "string", "invalid Test#undo(undo)");
//}@assert

    global[undo.name] = undo.base; // global["base"] = base;
    return null;
}
